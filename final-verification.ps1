# Final Verification Script - Memastikan rebrand Indonesia Daily selesai dengan baik

$WorkspaceRoot = "c:\KULIAH\MAGANG\Magang di Perhutani\Indonesia Daily"

Write-Host "========== FINAL VERIFICATION - Indonesia Daily REBRAND ==========" -ForegroundColor Cyan
Write-Host ""
...
$oldBrandingPatterns = @("Kabar Purwasuka", "KabarPurwasuka", "KabarPurwasuka", "kabarpurwasuka@gmail.com")
...
Write-Host "4. Checking for new branding 'Indonesia Daily'..." -ForegroundColor Yellow
$newBrandingFound = Get-ChildItem -Path $WorkspaceRoot -Recurse -Include "*.html" -File |
    Select-String -Pattern "Indonesia Daily|indonesiadaily|indonesiadaily" -ErrorAction SilentlyContinue |
    Where-Object { $_.Path -notlike "*\.bak.*" } | 
    Measure-Object

if ($newBrandingFound.Count -gt 0) {
    Write-Host "   ✅ Found 'Indonesia Daily' branding in $($newBrandingFound.Count) places" -ForegroundColor Green
} else {
    Write-Host "   ⚠️  No 'Indonesia Daily' branding found!" -ForegroundColor Yellow
    $issues += @{ Type = "Missing"; File = "All"; Reason = "No Indonesia Daily branding found" }
}
...
foreach ($pkg in $pkgFiles) {
    ...
    if ($content -match '"name"\s*:\s*"indonesiadaily') {
        $pkgOK++
